...
export class DaggerHeartCorePlugin implements VTTPlugin {
  ...
  onActivate(sdk: IPluginSDK): void {
    ...
    sdk.ui.registerRenderer(
      ...
      DHActionCheckCard as any,
    )

    // Define workflow with 5 steps: modifier → roll → judge → emit → resolve
    this.actionCheckHandle = sdk.defineWorkflow<ActionCheckData>(
      'daggerheart-core:action-check',
      [
        {
          id: 'modifier',
          run: async (ctx) => {
            ...
            const result = await ctx.requestInput<ModifierResult>(
              'daggerheart-core:modifier',
        ...
        },
        {
          id: 'resolve',
          run: (ctx) => {
            const judgment = ctx.vars.judgment as
