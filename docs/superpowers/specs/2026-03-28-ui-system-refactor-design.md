...
// 预定义通道
const tokenDrag = createAwarenessChannel<{ tokenId: string; x: number; y: number }>(
  'core:token.drag',
)
const resourceEdit = createAwarenessChannel<{ entityId: string; field: string; value: number }>(
  'core:resource.edit',
)

// 插件自定义通道
const spellTargeting = createAwarenessChannel<{ tokenIds: string[]; range: number }>(
  'dh:spell.targeting',
)
```
