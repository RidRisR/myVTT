...
> **状态**：实施中 | 2026-03-31 更新（UI System Phase 3 完成 — Input Handler + SDK 响应式 Hooks）
...
graph TD
    ...
    subgraph DiceSys["diceSystem（1 处 🟡 — 已大幅精简）"]
        D1["evaluateRoll + getJudgmentDisplay<br/>→ RollResultRenderer"]
    end
...
| 骰子系统     | ~~RulePlugin.diceSystem~~ 已精简为 evaluateRoll+getJudgmentDisplay | workflow + command + RendererRegistry |
| 跨客户端效果 | EventBus（仅执行者）                                               | 日志渲染器驱动（RendererRegistry）    |
...
- **diceSystem 接口** — 已精简：`getRollActions`/`getDieStyles`/`getModifierOptions`/`rollCommands`/`rollWorkflows` 已删除；仅保留 `evaluateRoll`+`getJudgmentDisplay`（供 RollResultRenderer 调用），待后续迁移到 workflow step
