...
> **状态**：实施中 | 2026-03-30 更新（Sprint 2b 渲染统一完成）
...
graph TD
    ...
    Sprint1 --> Sprint2 --> Sprint2b --> Sprint3 --> Sprint4
...
3. **Sprint 3**：修复跨客户端效果 bug（EventBus 迁移）+ 继续精简 RulePlugin（删 `hideElements`）
4. **Sprint 4+**：操作 workflow 化，独立轨道，逐步推进；`diceSystem.evaluateRoll`/`getJudgmentDisplay` 迁移为 workflow step
