...
graph TB
    ...
    subgraph EngineLayer["Engine 层（永驻）"]
        ...
        CMR["CommandRegistry · ExtensionRegistry"]
...
| 跨客户端效果 | EventBus（仅执行者）   | 日志渲染器驱动（ExtensionRegistry） |
...
- **surfaces 对象** — 被 `sdk.ui.registerComponent` / `sdk.commands` / `sdk.ui.contribute` 等细粒度 API 替代
...
gantt
    ...
    布局引擎 (XL)                 :b1, 9, 14
    ExtensionRegistry + Awareness (M) :b2, 14, 16
    基座UI迁移 (XL)               :b3, 16, 22
    模组管理器 (XL)               :b4, 22, 27
...
graph TD
    ...
    B1["B5 布局引擎 + ExtensionRegistry + Awareness<br/>📦 XL · 🔓 无依赖<br/>⏸️ 低优先级"]
...
先实现 `logEntryRenderers`（轨道 A），ChatPanel 保留在基座但使用新渲染器。渲染器通过 ExtensionRegistry 的 `contribute()` 注册（见 UI spec §4.2），不依赖布局引擎。ChatPanel 可以晚一步插件化。
...
graph TD
    ...
    subgraph Target["目标路径 🟢"]
        GLE2["GameLogEntry"] --> RR["ExtensionRegistry.get(type)"] --> RENDER["渲染器组件"]
...
| Step | 内容                                                                     | 工作量 |
| ---- | ------------------------------------------------------------------------ | ------ |
| 1    | ExtensionRegistry 类（统一 Slot + Renderer）                             | S      |
| 2    | 基座默认渲染器（`core:text`、`core:roll-result`、`core:tracker-update`） | M      |
| 3    | 插件渲染器注册（`dh:judgment`），替代 `rollCardRenderers`                | M      |
| 4    | ChatPanel 迁移到 ExtensionRegistry                                       | L      |
| 5    | 因果链分组渲染（按 `parentId` 聚合）                                     | M      |
| 6    | 渲染器驱动跨客户端效果（合并 A4）                                        | M      |

**涉及文件**：新建 `src/ui/extensionRegistry.ts`；修改 `src/chat/ChatPanel.tsx`、`src/chat/MessageCard.tsx`；`src/workflow/pluginSDK.ts`
...
graph TD
    ...
    subgraph Sprint3["🟠 Sprint 3: 效果修复 ~4天"]
        ...
        RP1s["RulePlugin精简#1<br/>S · 1天<br/>删hideElements<br/>rollCardRenderers→ExtensionRegistry"]
    end

    subgraph Sprint4["🔵 Sprint 4+: 操作WF化"]
        C2s["C2 DH Batch1<br/>L · 5-7天"]
        RP2s["RulePlugin精简#2<br/>S · 1天<br/>diceSystem→CommandTrigger"]
    end
