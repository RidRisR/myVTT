...
> ⚠️ 2026-04 审计注记：本文档的三轨演进模型已因 RulePlugin 提前退役而部分失效。后续演进方向见 [20-演进方向修正](20-演进方向修正.md)。

> **状态**：✅ RulePlugin 已完全退役（commit aa5e9d5）。VTTPlugin 单一架构已达成。| 2026-04-07 更新
...
graph TD
    ...
    subgraph Sprint3["🟠 Sprint 3: 效果修复 + 精简（⚠️ 2026-04 更新见下方注记）"]
        A3bs["✅ A3 Step5-6 + A4<br/>M · 2-3天<br/>EventBus 跨客户端迁移（PR #179 删除 EventBus）"]
        RP1s["✅ RulePlugin精简#1<br/>S · 1天<br/>删hideElements（RulePlugin 退役时一并移除）"]
        FT1["❌ formulaTokens 废弃<br/>S · Issue #182<br/>仍在 baseWorkflows.ts 活跃使用，不可废弃"]
    end

    subgraph Sprint4["🔵 Sprint 4+: 操作WF化（⚠️ 2026-04 更新见下方注记）"]
        C2s["C2 DH Batch1<br/>L · 5-7天"]
        RP2s["✅ RulePlugin精简#2<br/>S · 1天<br/>diceSystem 已删除，DiceJudge 替代"]
    end
...
4. **Sprint 3**：修复跨客户端效果 bug（EventBus 迁移）+ 继续精简 RulePlugin（删 `hideElements` + 废弃 `formulaTokens`）
   > ⚠️ 2026-04 更新：A3 Step5-6 + A4 EventBus → ✅（PR #179 删除 EventBus）；hideElements → ✅（RulePlugin 退役时一并移除）；formulaTokens 废弃 → ❌（仍在 baseWorkflows.ts 活跃使用，不可废弃）
5. **Sprint 4+**：操作 workflow 化，独立轨道，逐步推进；`diceSystem.evaluateRoll`/`getJudgmentDisplay` 迁移为 workflow step
   > ⚠️ 2026-04 更新：diceSystem 精简 → ✅（已删除，DiceJudge 替代）
