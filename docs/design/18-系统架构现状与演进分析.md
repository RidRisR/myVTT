...
> ⚠️ 2026-04 审计注记：部分评估已过时，已在文内标注更新。后续演进方向见 [20-演进方向修正](20-演进方向修正.md)。
...
graph TB
    ...
    subgraph Server["服务端"]
        RT["③ 实时同步<br/>Socket.io 44+ events"]
...
| --- | ----------------------- | ------------- | -------- | ------ | --------------------------------------------------------------------------------------------------------------------------------- |
| ①   | 数据层（SQLite + REST） | ✅ 生产就绪   | 低       | —      | `server/schema.ts`, `server/db.ts`, `server/routes/*.ts`（⚠️ 2026-04：已增至 21 张表）                                            |
| ②   | 状态管理（zustand）     | ✅ 生产就绪   | 中       | 中     | `src/stores/worldStore.ts` (~1000 行)（⚠️ 2026-04：已增至 5 stores，补 layoutStore）                                              |
...
| ⑧   | 事件日志（game_log）    | ✅ 已完成     | 低       | —      | `server/logHandler.ts`, `src/workflow/logStreamDispatcher.ts`（⚠️ 2026-04：Dispatcher/TriggerRegistry/RendererRegistry 全部完成） |
| ⑨   | 插件系统                | ✅ 已完成     | 低       | —      | 见 `17-插件系统演进路线.md`（⚠️ 2026-04：RulePlugin 已完全退役，VTTPlugin 单一架构）                                              |
...
**现状**：4 个 zustand store（`worldStore` ~1000 行、`identityStore` ~200 行、`assetStore` ~100 行、`uiStore` ~100 行）。35 个 Socket.io 事件监听器。悲观更新为主，仅 3 处乐观更新。

> ⚠️ 2026-04 更新：已增至 5 stores（补 layoutStore）。
...
**现状**：38+ 个数据广播事件 + 6 个感知事件（cursor、tokenDrag、presence）。Socket.io 4.8 connection recovery（2 分钟窗口）。服务端中继感知事件不持久化。
...
### 3.7 聊天系统 — 🟡 已统一到 game_log，渲染器待插件化

**现状**：

`chat_messages` 表已移除。ChatPanel 已统一读取 `game_log`，通过 `logEntryToChatMessage()` 将日志条目转换为 ChatMessage 格式渲染。

```mermaid
graph LR
    subgraph Current["当前：统一 game_log"]
        GL["game_log 表<br/>所有消息类型"]
        CONV["logEntryToChatMessage()<br/>硬编码转换"]
        CP["ChatPanel<br/>遍历 logEntries"]
    end
```
...
| 问题                                      | 影响                       |
| ----------------------------------------- | -------------------------- |
| `logEntryToChatMessage()` 硬编码转换      | 额外的映射代码，类型不安全 |
| ChatPanel 渲染路径独立于 RendererRegistry | 无法统一由插件贡献渲染器   |

**演进方向**：实现 RendererRegistry，替代 `logEntryToChatMessage()` 硬编码转换（与 doc 17 轨道 A 协同）：
...
- RendererRegistry 是插件轨道 A（doc 17 A3）的一部分
- 完成后 `logEntryToChatMessage()` 可移除

---

### 3.8 事件日志 — ✅ 已完成

> ⚠️ 2026-04 更新：所有未完成项已全部完成。Dispatcher 运行时接入、RendererRegistry、EventBus 移除均已在 Sprint 2-3 + DaggerHeart 验收中完成。
...
- ✅ EventBus 已移除（PR #179，由 RendererRegistry + game_log 替代）
...
### 3.9 插件系统 — ✅ RulePlugin 已完全退役

> ⚠️ 2026-04 更新：RulePlugin 已在 commit aa5e9d5 中完全退役。VTTPlugin 单一架构已达成。

详见 `17-插件系统演进路线.md`。核心演进方向：

- ~~三条轨道：事件日志完善 → UI 插件化 → 操作 Workflow 化~~
- ✅ 终态已达成：只有 VTTPlugin，RulePlugin 已退役
- ~~4 个 Sprint，预计 ~20 天~~
...
| 可并行                                                  | 有序列依赖                                |
| ------------------------------------------------------- | ----------------------------------------- |
| 类型安全 Phase 1-2 ↔ 资产存储 ↔ 可观测性 ↔ 状态管理拆分 | 日志渲染器（含 ChatPanel 迁移）→ EventBus |
...
3. **中期（Day 11-18）**：插件 Sprint 2-3 完成日志渲染器（RendererRegistry），ChatPanel 从 `logEntryToChatMessage()` 硬编码转换迁移到插件化渲染。战术先攻系统独立推进。
...
| **P2**   | 资产存储抽象                                 | 云部署前置条件                      |
| **P2**   | RendererRegistry（ChatPanel 插件化渲染）     | 插件轨道 A 的一部分（doc 17 A3）    |
