...
> **状态**：规划中 | 2026-04-04（基于 PR #187 合并后更新）
...
| Phase | 前置依赖 | 工作量 | 涉及消费点 |
|---|---|---|---|
| 0 | 无 | 0 | hideElements |
| 1 | 无 | L（1c 实体创建工作流化涉及 9 条路径精简为 3 + 服务端重构） | panels, teamPanel, dataTemplates, i18n (5 处) |
| 2 | UIRegistry 扩展 contribute API | M | tools, tokenActions, contextMenu (7 处) |
| 3 | RendererRegistry slot 模式（doc 20） | L-XL | EntityCard, adapters 全部 (24 处) |
| 4 | Phase 0-3 全部完成 | S | 基础设施删除 |
...
export const ENTITY_CARD_POINT = createRendererPoint<React.ComponentType<EntityCardProps>>('portrait', 'entity-card')
```
