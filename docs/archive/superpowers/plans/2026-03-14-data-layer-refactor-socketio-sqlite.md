...
export function sceneRoutes(dataDir: string, io: Server): Router {
  ...
  router.patch('/api/rooms/:roomId/scenes/:id', room, (req, res) => {
    const existing = req
      .roomDb!.prepare('SELECT * FROM scenes WHERE id = ?')
  ...
  router.post('/api/rooms/:roomId/scenes/:sceneId/entities/:entityId', room, (req, res) => {
    req
      .roomDb!.prepare('INSERT OR IGNORE INTO scene_entities (scene_id, entity_id) VALUES (?, ?)')
...
export function entityRoutes(dataDir: string, io: Server): Router {
  ...
  router.post('/api/rooms/:roomId/entities', room, (req, res) => {
    ...
    if (persistent) {
      ...
      const insert = req.roomDb!.prepare(
...
export function encounterRoutes(dataDir: string, io: Server): Router {
  ...
  router.get('/api/rooms/:roomId/scenes/:sceneId/encounters', room, (req, res) => {
    const encounters = req
      .roomDb!.prepare('SELECT * FROM encounters WHERE scene_id = ?')
...
export function combatRoutes(dataDir: string, io: Server): Router {
  ...
  router.patch('/api/rooms/:roomId/combat', room, (req, res) => {
    ...
    if (fields.length > 0) {
      req
        .roomDb!.prepare(`UPDATE combat_state SET ${fields.join(', ')} WHERE id = 1`)
        .run(...values)
    }
  ...
  router.post('/api/rooms/:roomId/combat/tokens', room, (req, res) => {
    ...
    req
      .roomDb!.prepare('UPDATE combat_state SET tokens = ? WHERE id = 1')
...
export function chatRoutes(dataDir: string, io: Server): Router {
  ...
  router.post('/api/rooms/:roomId/chat/retract/:id', room, (req, res) => {
    ...
    if (original.type === 'roll')
      return res.status(400).json({ error: 'Cannot retract roll messages' })
...
init: async (roomId, socket) => {
  ...
  const [scenesArr, entitiesArr, chat, combat, trackers, state, assets, showcase] =
    await Promise.all([
  ...
  socket.on('entity:created', ({ entity }: { entity: Entity }) =>
    set((s) => ({
      entities: { ...s.entities, [entity.id]: entity },
    })),
  )
  socket.on('entity:updated', ({ id, changes }: { id: string; changes: Partial<Entity> }) =>
    set((s) => {
      ...
      if (changes.permissions)
        merged.permissions = deepMerge(existing.permissions, changes.permissions)
...
if (!roomId) {
  console.error('Usage: tsx scripts/migrate-ydoc-to-sqlite.ts <roomId>')
  process.exit(1)
}
...
async function migrate() {
  ...
  yScenes.forEach((sceneMap: any, sceneId: string) => {
    ...
    db.prepare(
      'INSERT OR IGNORE INTO scenes (id, name, sort_order, atmosphere) VALUES (?, ?, ?, ?)',
    ).run(
      sceneId,
      sceneMap.get('name') || '',
      sceneMap.get('sortOrder') || 0,
      JSON.stringify(atmosphere),
    )
    ...
    if (entityIds instanceof Y.Map) {
      entityIds.forEach((_: any, eid: string) => {
        db.prepare('INSERT OR IGNORE INTO scene_entities (scene_id, entity_id) VALUES (?, ?)').run(
          sceneId,
          eid,
        )
      })
  ...
  yEntities.forEach((yMap: any, entityId: string) => {
    ...
    if (permYMap instanceof Y.Map) {
      ...
      if (seatsYMap instanceof Y.Map)
        seatsYMap.forEach((v: any, k: string) => {
          seats[k] = v
        })
    ...
    if (ruleYMap instanceof Y.Map) {
      ruleYMap.forEach((v: any, k: string) => {
        ;(ruleData as any)[k] = v
      })
    }
  ...
  for (const msg of yChat.toArray()) {
    ...
    chatInsert.run(
      ...
      m.content || null,
      m.type === 'roll'
        ? JSON.stringify({ expression: m.expression, terms: m.terms, total: m.total })
        : null,
  ...
  ySeats.forEach((seat: any, seatId: string) => {
    ...
    db.prepare('INSERT OR IGNORE INTO seats (id, name, color, role) VALUES (?, ?, ?, ?)').run(
      seatId,
      seat.name || '',
      seat.color || '#3b82f6',
      seat.role || 'PL',
  ...
  yMetrics.forEach((tracker: any, trackerId: string) => {
    ...
    db.prepare(
      'INSERT OR IGNORE INTO team_trackers (id, label, current, max, color, sort_order) VALUES (?, ?, ?, ?, ?, ?)',
    ).run(
      trackerId,
      tracker.label,
      tracker.current || 0,
  ...
  yShowcase.forEach((item: any, itemId: string) => {
    ...
    db.prepare(
      'INSERT OR IGNORE INTO showcase_items (id, type, data, pinned) VALUES (?, ?, ?, ?)',
...
socket.on('scene:updated', ({ id, changes }) =>
  set((s) => ({
...
// 场景-实体关联
socket.on('scene:entity:linked', ({ sceneId, entityId }) =>
  set((s) => ({
    sceneEntityMap: {
      ...s.sceneEntityMap,
      [sceneId]: [...(s.sceneEntityMap[sceneId] || []), entityId],
    },
  })),
)
socket.on('scene:entity:unlinked', ({ sceneId, entityId }) =>
  set((s) => ({
    sceneEntityMap: {
      ...s.sceneEntityMap,
      [sceneId]: (s.sceneEntityMap[sceneId] || []).filter((id) => id !== entityId),
    },
  })),
)
...
socket.on('entity:updated', ({ id, changes }) =>
  set((s) => {
    const existing = s.entities[id]
    if (!existing) return s
...
socket.on('combat:updated', ({ changes }) =>
  set((s) => ({
...
socket.on('combat:token:added', ({ token }) =>
  set((s) => {
    if (!s.combatState) return s
    return {
      combatState: { ...s.combatState, tokens: { ...s.combatState.tokens, [token.id]: token } },
    }
  }),
)
socket.on('combat:token:updated', ({ tokenId, changes }) =>
  set((s) => {
    if (!s.combatState) return s
    ...
    return {
      combatState: {
        ...s.combatState,
        tokens: { ...s.combatState.tokens, [tokenId]: { ...existing, ...changes } },
      },
    }
  }),
)
socket.on('combat:token:removed', ({ tokenId }) =>
  set((s) => {
    if (!s.combatState) return s
...
// 展示
socket.on('showcase:created', ({ item }) =>
  set((s) => ({ showcaseItems: [...s.showcaseItems, item] })),
)
socket.on('showcase:updated', ({ id, changes }) =>
  set((s) => ({
    showcaseItems: s.showcaseItems.map((i) => (i.id === id ? { ...i, ...changes } : i)),
  })),
)
socket.on('showcase:deleted', ({ id }) =>
  set((s) => ({
...
// 房间状态
socket.on('room:state:updated', ({ changes }) =>
  set((s) => ({
    room: { ...s.room, ...changes },
  })),
)
...
// 团队追踪器
socket.on('tracker:created', ({ tracker }) =>
  set((s) => ({
    teamTrackers: [...s.teamTrackers, tracker],
  })),
)
socket.on('tracker:updated', ({ id, changes }) =>
  set((s) => ({
...
socket.on('asset:updated', ({ id, changes }) =>
  set((s) => ({
...
// 新 selectIsCombat
export const selectIsCombat = (s: { room: RoomState }): boolean => s.room.activeEncounterId != null
```
...
yBlueprints.forEach((bp: any, bpId: string) => {
  ...
  db.prepare(
    'INSERT OR IGNORE INTO assets (id, url, name, type, created_at, extra) VALUES (?, ?, ?, ?, ?, ?)',
  ).run(
    bpId,
    bp.imageUrl || '',
    bp.name || '',
    ...
    JSON.stringify({
      ...
      defaultColor: bp.defaultColor,
      defaultRuleData: bp.defaultRuleData,
    }),
  )
})
