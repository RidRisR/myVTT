...
| 拖拽用 capture phase (`onPointerDownCapture`)，不依赖 bubble             | 子组件可能 `stopPropagation` 阻断冒泡        | `FloatingCard.tsx:L170`     |
| `useClickOutside` mount-frame guard (`requestAnimationFrame`)            | 防止触发 mount 的 pointerdown 被自身捕获     | `useClickOutside.ts:L23-26` |
