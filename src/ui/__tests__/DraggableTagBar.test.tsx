...
describe('DraggableTagBar', () => {
  ...
  it('shows Clear button when tags are selected', () => {
    renderWithDnd(
      <DraggableTagBar tags={['fantasy', 'dark']} selectedTags={['fantasy']} onToggleTag={vi.fn()} />,
    )
  ...
  it('does not show Clear button when no tags selected', () => {
    renderWithDnd(
      <DraggableTagBar tags={['fantasy']} selectedTags={[]} onToggleTag={vi.fn()} />,
    )
  ...
  it('calls onToggleTag for each selected tag when Clear is clicked', () => {
    ...
    renderWithDnd(
      <DraggableTagBar tags={['a', 'b', 'c']} selectedTags={['a', 'c']} onToggleTag={onToggleTag} />,
    )
    fireEvent.click(screen.getByText('Clear'))
    expect(onToggleTag).toHaveBeenCalledTimes(2)
    // forEach passes (value, index, array) — verify the first arg
    expect(onToggleTag.mock.calls[0]![0]).toBe('a')
    expect(onToggleTag.mock.calls[1]![0]).toBe('c')
  })
